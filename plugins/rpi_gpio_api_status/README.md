...
| **在线配置** | 改代码或手动编辑文件 | 飞书卡片一键测试连通性、一键切换网关 URL / Token |
...
| `/led config <url> [token]` | 快速修改并保存网关 URL 与 Token（例如 `/led config http://127.0.0.1:8080 ipad_pro_secret_888`） |
...
1. **🔌 测试连通性**：点击卡片底部的【🔌 测试连通性】按钮，即时检测网关状态并在卡片顶部显示响应延迟（ms）、HTTP 状态码及驱动模式。
2. **⚙️ 配置 URL/Token**：点击【⚙️ 配置 URL/Token】按钮，进入配置视图，支持一键切换本地网关 (`http://127.0.0.1:8080`) 或生产域名 (`https://piled.abab.pw`)。
